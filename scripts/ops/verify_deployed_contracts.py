...
async def verify_infrastructure():
    ...
    if not quota_manager.can_make_request("eth_getCode"):
        print("[FAIL] RPC quota would be exceeded. Aborting verification.")
        return False

    # 1. Network
    if await w3.is_connected():
        block = await w3.eth.block_number
        print(f"[OK] Connected to Polygon. Current Block: {block}")
        quota_manager.record_request("eth_blockNumber")
    else:
        print("[FAIL] Failed to connect to Polygon RPC.")
        return False
    ...
    if len(hft_code) > 2:
        print(f"[OK] HFT/C1 Contract Verified at {HFT_ADDRESS}")
        print(f"    - Bytecode detected ({len(hft_code)} bytes).")
        quota_manager.record_request("eth_getCode")
    else:
        print(f"[FAIL] HFT/C1 Contract MISSING or INVALID at {HFT_ADDRESS}")

    # 3. Liquidation Contract
    quota_manager.sync_wait_if_needed("eth_getCode")
    liq_code = await w3.eth.get_code(LIQ_ADDRESS)
    if len(liq_code) > 2:
        print(f"[OK] Liquidation Contract Verified at {LIQ_ADDRESS}")
        print(f"    - Bytecode detected ({len(liq_code)} bytes).")
        quota_manager.record_request("eth_getCode")
    else:
        print(f"[FAIL] Liquidation Contract MISSING or INVALID at {LIQ_ADDRESS}")
