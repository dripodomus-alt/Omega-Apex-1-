...
from . import rpc_layer  # This should be the single source of truth for RPCs
from .config import (  # Keep non-RPC config
    CHAIN_ID,
...
from .transport_lanes import dodo_endpoint_metadata, transport_status
...
def _probe_broadcast_rpc() -> tuple[bool, str]:
    if not rpc_layer.BROADCAST_RPC_URL:
        return False, "not configured"
    try:
        from web3 import Web3
        probe = Web3(Web3.HTTPProvider(rpc_layer.BROADCAST_RPC_URL, request_kwargs={"timeout": 10}))
...
def main() -> int:
    ...
    print(f"broadcast_rpc={'SET' if rpc_layer.BROADCAST_RPC_URL else 'NOT SET'}")
    print(f"broadcast_wss={'SET' if rpc_layer.LISTENER_WSS_URLS else 'NOT SET'}")
    ...
    dodo_count = len(dodo_endpoint_metadata())
    ...
    fork_url = rpc_layer.DISCOVERY_RPC_URL
    fork_detail = "rpc_layer.DISCOVERY_RPC_URL"
    ...
    _, token_list_stats = fetch_polygon_pos_candidates(known_addresses=[], known_symbols=[])
    print(f"polygon_token_list_stats={token_list_stats}")
