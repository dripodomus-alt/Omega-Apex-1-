...
def quote_algebra_clmm(
    ...
    """Placeholder for Algebra (QuickSwap V3) quote - reuses V3 logic for now."""
...
def verify_buy_low_sell_high_sequence(route: Dict[str, Any]) -> bool:
    ...
    # Core check for 2-hop (extendable to 3/4-hop cumulative product > 1.0)
    try:
        if len(steps) >= 2:
            # Expect explicit BUY_LEG1_PRICE and SELL_LEG2_PRICE in steps (per unified schema)
            buy_price = Decimal(str(steps[0].get("BUY_LEG1_PRICE") or steps[0].get("price", 0)))
            sell_price = Decimal(str(steps[1].get("SELL_LEG2_PRICE") or steps[1].get("price", 0)))
            if sell_price <= buy_price * Decimal("1.0001"):  # small tolerance for fees
                logger.debug(f"Sequence proof failed: sell_price={sell_price} <= buy_price={buy_price}")
                return False
        # For multi-hop, check cumulative direction or net positive
        cumulative = Decimal("1")
        for step in steps:
            p = Decimal(str(step.get("price", step.get("amount_out", 1))))
            cumulative *= p
