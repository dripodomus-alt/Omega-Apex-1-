...
def _inject_poa_middleware(provider: HTTPProvider) -> Web3:
    """Build a Web3 client with Polygon-compatible POA middleware."""
    client = Web3(provider)
    try:
        client.middleware_onion.inject(ExtraDataToPOAMiddleware, layer=0)
    except Exception:
        # Middleware may already be present depending on web3 version/runtime.
        pass
    return client
...
def connect(
    ...
    """Backward-compatible RPC connector used across runtime tools."""
    del wss_url, prefer_wss  # WSS is intentionally ignored in this compatibility layer.
    global w3, BLOCK, RPC_LIVE

    candidates = [u for u in (http_urls or []) if _is_usable_rpc_url(u)]
    if not candidates and _is_usable_rpc_url(DISCOVERY_RPC_URL):
        candidates.append(DISCOVERY_RPC_URL)
    if not candidates and _is_usable_rpc_url(HTTP_URL):
        candidates.append(HTTP_URL)

    for url in candidates:
        try:
            provider = QuotaAwareHTTPProvider(url)
            probe = _inject_poa_middleware(provider)
            if probe.is_connected():
                w3 = probe
                RPC_LIVE = True
                BLOCK = int(probe.eth.block_number)
                return True
        except Exception:
            continue
...
def get_w3_instance_for_chain(chain_id: int) -> Web3:
    """Provide a chain-scoped Web3 instance for pricing/oracle integrations."""
    if int(chain_id) != int(CHAIN_ID):
        raise ValueError(f"unsupported chain_id={chain_id}; configured chain_id={CHAIN_ID}")
    if w3 is not None and w3.is_connected():
        return w3
    if not connect(http_urls=[DISCOVERY_RPC_URL, HTTP_URL], wss_url="", prefer_wss=False):
        raise RuntimeError("RPC is not connected")
    return w3  # type: ignore[return-value]


def get_executor_address() -> str:
    """Return canonical executor target address used by staging envelopes."""
    candidate = C1_PAYLOAD_TARGET or EXECUTOR_CONTRACT
    if candidate and Web3.is_address(candidate):
        return Web3.to_checksum_address(candidate)
    return "0x0000000000000000000000000000000000000000"


def get_latest_gas_prices() -> dict[str, Decimal]:
    """Return current gas pricing in wei and gwei for sizing/profitability modules."""
    try:
        client = get_w3_instance_for_chain(CHAIN_ID)
        wei = Decimal(int(client.eth.gas_price))
    except Exception:
        wei = Decimal("30000000000")  # 30 gwei fallback
    return {
        "gas_price_wei": wei,
        "gas_price_gwei": wei / Decimal("1000000000"),
    }
