...
STREAM_HOLDING_TANK = "omega:queue:holding_tank"
...
LANE_SIMULATION_FROM_ADDRESS = 32
...
LANES: dict[int, Lane] = {
    ...
    32: Lane(32, "simulation_from_address", "read", STREAM_RPC_HEALTH, 1, "read"), # Low RPS, just needs a wallet
}
...
def simulation_from_address() -> str:
    ...
    from .config import OWNER_ADDRESS, C1_PAYLOAD_TARGET

    # In order of preference: the configured hot wallet, the owner, or the contract itself.
    # This ensures simulations run with a valid 'from' address.
    return wallet_address() or OWNER_ADDRESS or C1_PAYLOAD_TARGET or ""


def record_for_holding_tank(payload: dict[str, Any]) -> str:
    """Records a fully staged and verified transaction to the final submission queue."""
    return redis_cache.xadd(STREAM_HOLDING_TANK, payload)
