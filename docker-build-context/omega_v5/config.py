...
def normalize_protocol(protocol: str) -> str:
    ...
    if "CURVE" in p:
        return "CURVE_STABLE"
    if "KYBER" in p:
        return "KYBER_ELASTIC"
    if "DODO" in p:
        return "DODO_PMM"
    if "UNISWAPV4" in p or "V4" in p:
        return "UNISWAP_V4"
    return p
...
def _coerce_decimal(value: Any) -> Decimal:
    if value is None or value == "":
        raise ValueError("fee value is empty")
    ...
    if isinstance(value, (int, float, str)):
        try:
            ...
            raise ValueError(f"invalid fee value: {value}") from exc
    raise TypeError(f"unsupported fee value type: {type(value)!r}")


def resolve_protocol_fee_fraction(
    protocol: Any,
    fee_value: Any = None,
    *,
    fee_kind: str = "auto",
    ...
    """Resolve a protocol-specific swap fee fraction from protocol metadata and fee inputs.

    The mapper is intentionally conservative:
    - explicit fee fractions are preserved as-is,
    - explicit bps values are converted using 1/10000,
    - explicit fee-tier values are converted using 1/1_000_000,
    - when no fee input is provided, protocol-family defaults are used.
    """
    normalized = normalize_protocol(str(protocol or ""))
    if default is not None:
        return _coerce_decimal(default)

    default_fractions = {
        "V2_CPMM": Decimal("0.003"),
        "QS_V2_CPMM": Decimal("0.003"),
        "SUSHI_V2_CPMM": Decimal("0.003"),
        "DFYN_V2_CPMM": Decimal("0.003"),
        "MESH_V2_CPMM": Decimal("0.003"),
        "V3_CLMM": Decimal("0.003"),
        "QS_V3_ALGEBRA": Decimal("0.003"),
        "CURVE_STABLE": Decimal("0.0004"),
        "BAL_WEIGHTED": Decimal("0.0004"),
        "KYBER_ELASTIC": Decimal("0.003"),
        "DODO_PMM": Decimal("0.003"),
        "UNISWAP_V4": Decimal("0.003"),
    }

    if fee_value is None:
        return default_fractions.get(normalized, Decimal("0.003"))

    value = _coerce_decimal(fee_value)
    normalized_kind = str(fee_kind or "auto").lower()

    if normalized_kind in {"fraction", "ratio", "fee_fraction"}:
        return value

    if normalized_kind in {"bps", "basis_points", "fee_bps", "basispoint"}:
        return value / Decimal("10000")

    if normalized_kind in {"fee_tier", "tier", "fee-tier"}:
        return value / Decimal("1000000")

    if normalized in {"V3_CLMM", "QS_V3_ALGEBRA", "UNISWAP_V4", "KYBER_ELASTIC"}:
        if value <= Decimal("1"):
            return value
        return value / Decimal("1000000") if value < Decimal("1000000") else value / Decimal("10000")

    if normalized in {"BAL_WEIGHTED", "CURVE_STABLE"}:
        if value <= Decimal("1"):
            return value
        return value / Decimal("10000")
...
def build_protocol_sequence_ids(route: Dict[str, Any]) -> List[int]:
    ...
    if hasattr(route, "get"):
        ...
        opp_id = route.get("opp_id") or route.get("opportunity_id") or "unknown"
    else:
        protocol_seq = getattr(route, "protocol_seq", None) or getattr(route, "protocol_sequence", None) or getattr(route, "protocols", None) or []
        opp_id = getattr(route, "opp_id", None) or getattr(route, "opportunity_id", None) or "unknown"

    if isinstance(protocol_seq, (str, bytes)):
    ...
    for p in protocol_seq:
        ...
        if pid is None:
            raise ValueError(f"No PROTOCOL_ID_MAP entry for '{key}' in route {opp_id}. Update registry first.")
        ids.append(pid)
    if not ids:
        raise ValueError("Empty protocol sequence in route")
    return ids
...
EXECUTOR_WALLET: str = _first_env("EXECUTOR_WALLET", "BOT_ADDRESS")
OWNER_ADDRESS: str = _first_env("OWNER_ADDRESS", "EXECUTOR_WALLET")
PROFIT_RECIPIENT_ADDRESS: str = _first_env("PROFIT_RECIPIENT_ADDRESS", "BOT_PROFIT_RECEIVER", "EXECUTOR_WALLET")
...
FASTLANE_RELAY_URL: str = _env("FASTLANE_RELAY_URL", _env("FASTLANE_RPC_URL", ""))
MIN_CALldata_LENGTH: int = int(_env("MIN_CALLDATA_LENGTH", "8") or "8")
MAX_CALldata_LENGTH: int = int(_env("MAX_CALLDATA_LENGTH", "65536") or "65536")
...
MIN_PROFIT_POL: Decimal = Decimal(_env("MIN_PROFIT_POL", "0") or "0")
...
ENABLE_DYNAMIC_POOL_REGISTRY: bool = _bool_env("ENABLE_DYNAMIC_POOL_REGISTRY", "true")
DYNAMIC_POOL_REGISTRY_MAX_POOLS: int = int(_env("DYNAMIC_POOL_REGISTRY_MAX_POOLS", "250") or "250")
DYNAMIC_POOLS_JSON_PATH: str = _env("DYNAMIC_POOLS_JSON_PATH", "omega_v5/data/dynamic_pools.json")
ENABLE_CURVE_POOL_REGISTRY: bool = _bool_env("ENABLE_CURVE_POOL_REGISTRY", "false")
CURVE_POOL_REGISTRY_API_BASE_URL: str = _env("CURVE_POOL_REGISTRY_API_BASE_URL", "https://api.curve.fi")
CURVE_POOL_REGISTRY_FAMILIES: List[str] = _csv_env("CURVE_POOL_REGISTRY_FAMILIES", "stable,crypto,factory")
CURVE_POOL_REGISTRY_MAX_POOLS: int = int(_env("CURVE_POOL_REGISTRY_MAX_POOLS", "100") or "100")
CURVE_POOL_REGISTRY_MIN_USD_TVL: Decimal = Decimal(_env("CURVE_POOL_REGISTRY_MIN_USD_TVL", "50000") or "50000")
ENABLE_POLYGON_TOKEN_LIST_DISCOVERY: bool = _bool_env("ENABLE_POLYGON_TOKEN_LIST_DISCOVERY", "true")
POLYGON_TOKEN_LIST_BASES: List[str] = _csv_env("POLYGON_TOKEN_LIST_BASES", "USDC,WETH,WPOL,USDT,DAI")
POLYGON_TOKEN_LIST_MAX_CANDIDATES: int = int(_env("POLYGON_TOKEN_LIST_MAX_CANDIDATES", "500") or "500")
ENABLE_SUBGRAPH_POOL_INTEL: bool = _bool_env("ENABLE_SUBGRAPH_POOL_INTEL", "false")
SUBGRAPH_POOL_INTEL_LIMIT: int = int(_env("SUBGRAPH_POOL_INTEL_LIMIT", "250") or "250")
...
DATA_INTEGRITY_CHECK_ENABLED: bool = _bool_env("DATA_INTEGRITY_CHECK_ENABLED", "true")
