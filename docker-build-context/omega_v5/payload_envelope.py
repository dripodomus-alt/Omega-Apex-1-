...
def build_payload_envelope(
    ...
    *,
    ...
    unique_salt: str = "",
    parent_envelope_id: str = "",
) -> PayloadEnvelope | dict[str, Any]:
    if opportunity is not None and kind is None and target is None and calldata is None:
        route = getattr(opportunity, "route", None) or opportunity.get("route", opportunity) if isinstance(opportunity, dict) else None
        if route is None:
            raise TypeError("opportunity must provide a route or a dict-like payload")
        opp_id = getattr(opportunity, "opp_id", None) or opportunity.get("opp_id", "unknown") if isinstance(opportunity, dict) else "unknown"
        envelope = UnifiedRouteEnvelope(str(opp_id), route if isinstance(route, dict) else dict(route))
        payload_stage = {
            "calldata": "0x...",
            "protocol_ids": build_protocol_sequence_ids(route if isinstance(route, dict) else dict(route)),
        }
        return envelope.with_stage("payload", payload_stage, status="PAYLOAD_READY").as_dict()

    if kind is None or target is None or calldata is None:
        raise TypeError("kind, target, and calldata are required when building a payload envelope")
