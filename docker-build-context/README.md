...
The system employs a segregated wallet security model to minimize risk during live operations. This is a critical concept to understand before deploying capital.

*   **`EXECUTOR_WALLET` (The "Hot Wallet"):**
    *   **Role:** This is the operational wallet "Ai Wazllet Agent" on the live VM. It is responsible for signing and broadcasting all transactions, such as arbitrage trades and liquidations.
    *   **Capitalization:** This wallet should **only** be funded with enough native tokens (e.g., POL on Polygon) to cover gas fees for a few days of operation. It should **not** hold significant capital or accumulated profits.
    *   **Security:** Because its key is used by an autonomous system, it is considered "hot" and has a higher risk profile. The system is designed to minimize the potential loss if this wallet were ever compromised.

*   **`PROFIT_RECIPIENT_ADDRESS` (The "Cold Wallet"):**
    *   **Role:** This is a secure address, such as a hardware wallet or a Gnosis Safe multi-sig, where all net profits are automatically sent. The private key for this wallet is **never** exposed to the VM or the application.
    *   **Capitalization:** This wallet's purpose is to accumulate profits safely. It does not sign transactions for the bot.
    *   **Security:** The `OmegaRouteSwapAdapter` smart contract is designed to automatically transfer any net profit from a trade directly to this address at the end of the transaction. This ensures that profits are immediately swept to a secure, offline location, protecting your capital.

This separation ensures that even in a worst-case scenario where the execution VM is compromised, the attacker cannot access the accumulated profits of the system. The pre-flight checks will enforce this separation when `EXECUTION_MODE` is set to `live`.
